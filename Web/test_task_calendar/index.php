<html>
  ...
  <style>
    ...
    #login {
      width: 20%;
      position: absolute; left: 75%; top: 2%;
      margin-left: 0;
    }
    #sort {
  ...
  <body>
    <?php
      ...
      if (isset($_POST['login'])) header('Location: action.php');
      ...
      if (isset($_SESSION['user_id']) and $_SESSION['user_id'] != 0){
        ...
        echo "<button type='submit' name='login'>Войти как Администратор</button>";
        echo "</form>";
      }
      ...
      if (!isset($_SESSION['regim']) or $_SESSION['regim'] == 'show'){
        ...
        if (isset($_SESSION['user_id']) and $_SESSION['user_id'] != 0) echo "<form action='#' method='post' id='empty'><button type='submit' name='create' >Создать запись</button></form>";

        echo "<form action='#' method='post'><table>";
        echo "<tr><th style='width: 97px;'>Выполнено</th><th style='width: 140px;'>Тип</th><th>Задача</th><th style='width: 120px;'>Место</th><th style='width: 120px;'>Дата завершения</th>";
        if (isset($_SESSION['user_id']) and $_SESSION['user_id'] != 0) echo "<th style='width: 86px;'></th><th style='width: 76px;'></th><th style='width: 98px;'></th>";
        echo "</tr>";

        $sql = "SELECT * FROM `affairs` WHERE `user_id` != 0";
